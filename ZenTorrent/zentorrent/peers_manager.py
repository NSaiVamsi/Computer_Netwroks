...
class PeersManager(Thread):
    def __init__(self, torrent, pieces_manager):
        ...
        self.is_active = True
        # self.pieces_manager1 = PiecesManager(self.torrent)
    ...
    # def peers_bitfield(self, peer):
    #     if not peer:
    #         logging.error("Peer is None in peers_bitfield method.")
    #         return

    #     for i in range(self.pieces_manager.number_of_pieces):
    #         if peer.has_piece(i) and peer not in self.pieces_by_peer[i][1]:
    #             self.pieces_by_peer[i][1].append(peer)
    #             self.pieces_by_peer[i][0] = len(self.pieces_by_peer[i][1])

    def rarest_piece1(self,counter):
        # ready_peers = [peer for peer in self.peers if peer.is_eligible() and peer.is_unchoked() and peer.am_interested()]
        ready_peers = []

        for peer in self.peers:
            if peer.is_eligible() and peer.is_unchoked() and peer.am_interested():
                ready_peers.append(peer)        

        for piece in self.pieces_manager.pieces:
            for peer in ready_peers:
                self.rarest_pieces.peers_bitfield(peer, piece.piece_index)

        sorted_pieces = self.rarest_pieces.get_sorted_pieces(counter)
        return sorted_pieces['idPiece'] if sorted_pieces else None
